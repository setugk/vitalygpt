...
## Hidden vs. Disabled vs.

📛 Hidden vs. Disabled vs. Read-Only (Decision Tree + PDF). Practical UX guidelines on when to hide, when to disable and when to keep a default state ↓ Decision Tree: https://lnkd.in/d8DijNi5
High-Res PDF/PNG: https://lnkd.in/da6vqsRP The question about the “right” state for UI components is a surprisingly tricky one. We know that hiding important features hurts their discoverability. Disabled features are frustrating without an explanation, especially with a fragile validation. And read-only items often look editable but can’t be changed. --- 📛 1. HIDDEN STATE A good first question to ask is if a component, or a feature should be fully available to a user, or rather limited in some way. Is it not useful or relevant? Is it not allowed for a specific user group? Is it incompatible, or unavailable? If the answer is No, we should probably keep it in its default state. But if it is limited somehow, then it’s not very useful if users will never be able to interact with it — be it by reading, selecting, copying, tabbing, clicking, or anything in-between. So if they won’t, we can safely hide the item — but then also design the paths on when and how and to whom we show it. --- 🤔 2. READ-ONLY STATE Mostly, we use read-only state only for text fields. Think of pre-generated invoice IDs, VAT tax calculation or an insurance number. If users can change them on other pages (e.g. in their Settings page), then we can safely use read-only state — perhaps with a hint on where to edit it. --- 🚫 3. DISABLED STATE Unlike read-only, features in a disabled state can’t be clicked, selected or interacted with, and they are inaccessible to keyboard users and a screen reader. We apply it when a user isn’t allowed to interact with it — due to permissions or temporary restrictions. Disabled buttons are helpful when they prevent critical mistakes, such as double transfers or accidental deletion. We use them mostly for actions — and usually by lowering opacity and also desaturating the colors with grey tones. But: they are difficult to read, and hard to interpret. They don’t have to be accessible according to WCAG 2.2 AA, but it shouldn’t be an excuse for an inaccessible experience. They should be readable, could be focusable, and must be accessible to screen readers. Disable If you want the user to know it exists but is unavailable. Hide if the value shown is currently irrelevant and can’t be used. But: never hide buttons or filters by default as users expect them to persist. And: disabled features help users learn the UI and understand benefits of an upgrade. Whenever possible, I try to keep buttons and features in their default state — enabled, accessible and legible. This way, when a user interacts with that feature, we can explain why they can’t use it, how to enable it and how to keep it enabled. #ux #design | 12 comments on LinkedIn

**Source:** [LinkedIn](https://www.linkedin.com/posts/vitalyfriedman_ux-design-share-7436302936647041025-7mS5/?utm_source=share&utm_medium=member_ios&rcm=ACoAAAedtLEBej9lid5LHaT1t_HU5bsQdM5QCJ0) · Vitaly Friedman · 2026-07-12
**Tags:** Accessibility, design-guidelines, ui-components, ux-design
